  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (p > 5 && p < 20) {
        ...
        story = [
          "FOUNDATION DETECTED: SRKR-CSE",
          "ARCHITECT ROLES: MERN-AI-DEVELOPER",
        ];
      } else if (p > 25 && p < 45) {
        ...
        story = [
          "PROJECTS DEPLOYED: NEXUS, HEALVERSE, DEEPNOX",
          "TECH STACK: MULTI-AGENT ARCHITECTURE",
        ];
      } else if (p > 75 && p < 90) {
      ...
      {isVisible && (
        <div className="fixed left-10 bottom-10 z-[1000] pointer-events-none select-none max-w-sm">
          ...
          <motion.div
            ...
            className="p-8 bg-[#0b0f2a]/80 backdrop-blur-3xl border border-border rounded-3xl overflow-hidden relative"
          >
            {" "}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex justify-between items-center">
                {" "}
                <span className="text-[10px] font-black uppercase text-mint tracking-[0.4em] animate-pulse">
                  {logStatus}
                </span>{" "}
                <span className="text-[8px] font-bold text-text-muted uppercase tracking-widest font-mono">
                  Archive // v1.0.4
                </span>{" "}
              </div>{" "}
              <div className="space-y-2 mt-4 min-h-16">
                ...
                {narrative.map((item, i) => (
                  <motion.div
                    ...
                    className="text-xs font-bold text-text-primary uppercase tracking-widest leading-relaxed opacity-60"
                  >
                    {" "}
                    <span className="text-mint/40 mr-3 italic">➔</span>{" "}
                    {item}{" "}
                  </motion.div>
                ))}{" "}
              </div>{" "}
