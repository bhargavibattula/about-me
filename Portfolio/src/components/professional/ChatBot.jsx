...
const ChatBot = () => {
  ...
  const [messages, setMessages] = useState([
    {
      type: "ai",
      text: "ARCHITECT_ASSISTANT v1.0 ONLINE. How can I assist with your inquiry?",
    },
  ]);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[10001]">
      ...
      <AnimatePresence>
        ...
        {isOpen && (
          <motion.div
            ...
            className="absolute bottom-20 right-0 w-[320px] md:w-[380px] h-[500px] bg-[#0A0B1E]/90 border border-border-strong rounded-3xl backdrop-blur-2xl shadow-2xl flex flex-col overflow-hidden"
          >
            {" "}
            {/* Header */}{" "}
            <div className="p-6 border-b border-border bg-bg-secondary flex items-center justify-between">
              {" "}
              <div className="flex items-center gap-3">
                {" "}
                <div className="w-2 h-2 rounded-full bg-mint animate-pulse shadow-[0_0_10px_#57db96]" />{" "}
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-text-primary/60">
                  Node Assistant // RX-9
                </span>{" "}
              </div>{" "}
              <button
            ...
            >
              ...
              {messages.map((m, i) => (
                ...
                >
                  {" "}
                  <div
                    className={`max-w-[85%] p-4 rounded-2xl text-[11px] leading-relaxed font-medium uppercase tracking-wider ${m.type === "ai" ? "bg-bg-secondary border border-border-strong text-text-primary/70" : "bg-mint text-[var(--accent-text)] font-black"}`}
                  >
                    {" "}
                    {m.text}{" "}
                  </div>{" "}
            ...
            {/* Quick Actions */}{" "}
            <div className="p-6 bg-black/40 border-t border-border">
              {" "}
              <div className="grid grid-cols-2 gap-2">
                ...
                {qaBank.map((item, i) => (
                  <button
                    ...
                    className="p-3 text-[8px] font-black uppercase tracking-widest text-text-muted border border-border rounded-xl hover:border-mint/40 hover:text-mint transition-all text-left truncate"
                  >
                    {" "}
                    {item.q}{" "}
                  </button>
      ...
      <motion.button
        ...
        className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all shadow-2xl border ${isOpen ? "bg-mint text-[var(--accent-text)] border-mint" : "bg-bg-secondary text-mint border-border-strong backdrop-blur-xl"}`}
      >
        {" "}
        {isOpen ? (
