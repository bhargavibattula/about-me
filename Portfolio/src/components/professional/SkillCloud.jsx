  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden select-none opacity-40">
      ...
      {nodes.map((node, i) => (
        ...
        >
          {" "}
          <div className="w-[1px] h-[1px] bg-[var(--text-primary)] shadow-[0_0_10px_var(--text-primary)] rounded-full" />{" "}
          <span className="text-[7px] font-black uppercase text-text-muted tracking-[0.2em]">
            {node.id}
          </span>{" "}
        </motion.div>
      ))}{" "}
      {/* Background Depth Layers */}{" "}
      <div className="absolute top-1/4 left-1/4 w-[800px] h-[800px] bg-royal/10 blur-[200px] rounded-full" />{" "}
      <div className="absolute bottom-1/4 right-1/4 w-[800px] h-[800px] bg-mint/10 blur-[200px] rounded-full" />{" "}
    </div>
  );
};
