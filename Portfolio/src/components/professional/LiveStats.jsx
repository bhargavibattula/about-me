...
const LiveStats = () => {
    ...
    const fetchStats = async () => {
      /* Fetch GitHub */ try {
        const ghRes = await fetch(
          "https://api.github.com/users/bhargavibattula",
        );
      ...
      /* Fetch LeetCode */ try {
        const lcRes = await fetch(
          "https://leetcode-stats-api.herokuapp.com/BhargaviTejaswi",
        );
  ...
  return (
    <section className="py-24 px-6 md:px-8 bg-bg-primary relative overflow-hidden">
      {" "}
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        <div className="mb-20 space-y-4 text-center md:text-left">
          ...
          <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter text-text-primary italic">
            {" "}
            Live Coding <span className="text-text-muted">Activity</span>{" "}
          </h2>{" "}
          <p className="text-[10px] font-black uppercase tracking-[0.6em] text-text-muted italic">
            Real-time developer performance & transmission diagnostics
          </p>{" "}
        </div>{" "}
            ...
            >
              ...
              <div className="relative z-10 flex flex-col h-full justify-between">
                ...
                <div className="flex items-center justify-between mb-12">
                  ...
                  <div className="px-3 py-1 rounded-full bg-bg-secondary border border-border-strong text-[8px] font-black uppercase tracking-widest text-text-muted group-hover:text-mint group-hover:border-mint transition-all">
                    {" "}
                    ⚡ Live{" "}
                  </div>{" "}
                </div>{" "}
