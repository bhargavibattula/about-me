...
const WelcomeScreen = () => {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[100000] bg-bg-primary flex flex-col items-center justify-center overflow-hidden"
    >
      {" "}
      {/* Background Depth */}{" "}
      ...
      <motion.div
        ...
        className="w-32 h-32 md:w-48 md:h-48 border border-border md:border-border-strong rounded-full flex items-center justify-center relative z-10 p-8 md:p-12 shadow-[0_0_80px_rgba(87,219,150,0.1)]"
      >
        {" "}
      ...
      <div className="text-center mt-24 md:mt-32 relative z-20 px-6">
        ...
        >
          {" "}
          <span className="text-text-primary">WEL</span>{" "}
          <span className="text-text-muted italic">COME</span>{" "}
        </motion.h1>{" "}
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.5 }}
          className="text-[7px] md:text-[12px] font-black uppercase tracking-[0.4em] md:tracking-[1em] text-text-muted italic max-w-xs md:max-w-none mx-auto leading-relaxed border-t border-border pt-6 inline-block"
        >
          {" "}
      ...
      <div className="absolute bottom-16 md:bottom-24 left-1/2 -translate-x-1/2 w-full max-w-[280px] md:max-w-xl px-4 z-20 flex flex-col items-center">
        {" "}
        <div className="flex justify-between w-full mb-4 md:mb-6 text-[8px] md:text-[10px] font-black uppercase tracking-widest text-text-muted">
          {" "}
          <span className="animate-pulse">{status}</span>{" "}
          <span className="text-mint/60">{progress}%</span>{" "}
        </div>{" "}
        <div className="w-full h-[2px] bg-bg-secondary rounded-full overflow-hidden">
          {" "}
          <motion.div
            initial={{ width: 0 }}
      ...
      <div
        ...
        style={{
          ...
          backgroundSize: "60px 60px md:100px 100px",
        }}
      />{" "}
    </motion.div>
  );
