...
const Terminal = () => {
  ...
  const processCommand = (cmd) => {
    ...
    if (cleanCmd === "help") {
      ...
      response = [
        ...
        {
          type: "output",
          content: "B.TECH IN COMPUTER SCIENCE AND ENGINEERING | CGPA: 9.22/10",
        },
      ];
    } else if (cleanCmd === "ls") {
      response = [
        {
          type: "output",
          content:
            "nexus_ai.bin healverse.apk deepfake_detector.py resume_dossier.pdf",
        },
      ];
      ...
      const files = {
        "nexus_ai.bin":
          "NEXUS AI (Feb 2026): Autonomous multi-agent platform for software lifecycle automation. Tech: Next.js, TS, FastAPI, Celery, LangGraph. | Repo: github.com/bhargavibattula/nexus-ai",
        "healverse.apk":
          "HealVerse (Aug 2025): AI-driven patient healthcare app with diet optimization and medication tracking. Tech: Java, Spring Boot, React Native. | Repo: github.com/bhargavibattula/healverse-server",
        "deepfake_detector.py":
          "DeepNox (Jan 2025): Deepfake detection browser extension with real-time trust scoring. Tech: Python, TensorFlow, OpenCV, Flask. | Repo: github.com/bhargavibattula/deep-nox",
        "resume_dossier.pdf":
          "ACCESS GRANTED: Battula Bhargavi. URL: https://drive.google.com/file/d/1pxrIHfK7MBrLd27p3Yd6dYcQMydDJKHY/view?usp=sharing",
      };
      response = [
        {
          type: "output",
          content: files[file] || `cat: ${file}: No such file or directory`,
        },
      ];
      ...
      response = [
        {
          type: "output",
          content: "☕ System caffeine levels increased by 400%.",
        },
      ];
      ...
      response = [
        {
          type: "error",
          content: `Command not found: ${cleanCmd}. Type "help" for options.`,
        },
      ];
    }
    setHistory((prev) => [
  ...
  const startHiringSequence = () => {
    ...
    setHistory((prev) => [
      ...prev,
      { type: "system", content: "CRITICAL ACCESS REQUESTED..." },
    ]);
  };
  ...
  return (
    <div
      className="h-full bg-black/40 p-6 font-mono text-sm relative flex flex-col"
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto mb-4 custom-scrollbar pr-2">
        ...
        {history.map((line, i) => (
          <div key={i} className="mb-1 leading-relaxed">
            ...
            {line.type === "system" && (
              <span className="text-fuchsia font-black">{line.content}</span>
            )}{" "}
            {line.type === "output" && (
              <span className="text-text-primary/60">{line.content}</span>
            )}{" "}
            {line.type === "error" && (
              <span className="text-coral italic">{line.content}</span>
            )}{" "}
          </div>
        ))}{" "}
        <div ref={scrollRef} />{" "}
      ...
      <div className="flex gap-2 shrink-0 border-t border-border pt-4">
        ...
        <input
          ...
          className="bg-transparent border-none outline-none text-text-primary flex-1 p-0 m-0"
          spellCheck="false"
        />{" "}
      </div>{" "}
                ...
                <motion.div
                  ...
                  onAnimationComplete={() => {
                    setHistory((prev) => [
                      ...
                      {
                        type: "system",
                        content: "EXCEPTIONAL DEVELOPER CONFIRMED",
                      },
      ...
      {/* Hint Bar */}{" "}
      <div className="h-6 border-t border-border-strong mt-2 flex items-center gap-4 text-[9px] uppercase font-black text-text-muted whitespace-nowrap overflow-hidden">
        {" "}
        {["help", "whoami", "ls", "cat", "hire tejaswi", "matrix", "clear"].map(
          (hint) => (
            <span
              key={hint}
              className="hover:text-text-primary/60 cursor-help transition-colors"
            >
              {hint}
            </span>
          ),
